(function(Monarch) {
  Monarch.Relations.Relation = new JS.Class('Monarch.Relations.Relation', {
    ...
    contents: function() {
      if (this.isActive) {
        if (!this._contents) {
          ...
          _.each(this._all(), function(tuple) {
            this.insert(tuple);
          }, this);
        }
    ...
    fetch: function() {
      return Monarch.Server.fetch(this);
    },
    ...
    remove: function(tuple, newKey, oldKey, changeset) {
      ...
      this._removeNode.publish(tuple, index, newKey, oldKey, changeset);
    },
    ...
    containsKey: function() {
      return _.any(arguments, function(key) {
    ...
    find: function(idOrPredicate) {
      var predicate = _.isObject(idOrPredicate) ? idOrPredicate : { id: idOrPredicate };
    ...
    where: function(predicate) {
      if (_.isEmpty(predicate)) return this;
    ...
    limit: function(limitCount, offsetCount) {
      var operand = offsetCount ? this.offset(offsetCount) : this;
    ...
    buildComparator: function(compareRecords) {
      ...
      return function(a, b) {
        for(var i = 0; i < length; i++) {
          ...
          var aValue = compareRecords ? a.getFieldValue(columnName) : a[columnName];
          var bValue = compareRecords ? b.getFieldValue(columnName) : b[columnName];
    ...
    buildOrderByExpressions: function(orderByStrings) {
      var orderByStrings = orderByStrings.concat(['id']);
      return _.map(orderByStrings, function(orderByString) {
    ...
    resolvePredicate: function(object) {
      ...
      var predicates = _.map(object, function(value, key) {
        return this.predicateForKeyValue(key, value);
      }, this);
    ...
    predicateForKeyValue: function(key, value) {
      var predicateClass;
      var parts = key.split(" ");
      if (parts[1]) {
        key = parts[0];
