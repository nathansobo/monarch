(function(Monarch) {
  Monarch.Relations.Relation = new JS.Class('Monarch.Relations.Relation', {
    ...
    remove: function(tuple, oldKey) {
      if (!oldKey) oldKey = this.buildKey(tuple);
      var index = this.contents().remove(oldKey);
      this._removeNode.publish(tuple, index, oldKey);
    },
