  ...
  Monarch.Relations.InnerJoin = new JS.Class('Monarch.Relations.InnerJoin', Monarch.Relations.Relation, {
    ...
    _activate: function() {
      ...
      this.subscribe(this.left, 'onUpdate', function(leftTuple, changeset, _, _, newKey, oldKey) {
        this.tupleUpdatedInOperand('left', leftTuple, changeset, oldKey);
      });

      this.subscribe(this.right, 'onUpdate', function(rightTuple, changeset, _, _, newKey, oldKey) {
        this.tupleUpdatedInOperand('right', rightTuple, changeset, oldKey);
      });
    },
    ...
    tupleUpdatedInOperand: function(side, tuple, changeset, oldKey) {
      this.each(function(composite) {
        if (composite[side] === tuple) {
          this.tupleUpdated(composite, changeset, this.buildKey(composite), this.buildKey(composite, oldKey));
        }
      }, this);
    },
