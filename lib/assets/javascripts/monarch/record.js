  ...
  Monarch.Record = new JS.Class('Monarch.Record', {
    extend: {
      ...
      syntheticColumn: function(name, definition) {
        this[name] = this.table.syntheticColumn(name, definition);
      ...
      hasMany: function(name, options) {
        ...
        var foreignKey = _.uncapitalize(this.table.name) + "Id";

        this.relatesTo(name, function() {
          var conditions = {};
          conditions[foreignKey] = this.id();
          return target.where(conditions);
        });
      },
    ...
    destroy: function() {
      ...
      return Monarch.Remote.Server.destroy(this)
        .onSuccess(this.method('afterDestroy'));
    },
