  ...
  describe("events", function() {
    ...
    describe("insert events", function() {
      it("triggers insert events when records are inserted into one operand that aren't in the other", function() {
        var post1 = BlogPost.remotelyCreated({id: 1, public: true, title: "Alpha", blogId: 2});
        ...
        var post2 = BlogPost.remotelyCreated({id: 2, public: false, title: "Bravo", blogId: 1});
        expect(insertCallback).toHaveBeenCalled();
    ...
    describe("update events", function() {
      it("triggers a single update event when records in the operands are updated, even the record is present in both", function() {
        var post1 = BlogPost.remotelyCreated({id: 1, public: false, title: "Alpha", blogId: 1});
        var post2 = BlogPost.remotelyCreated({id: 2, public: true, title: "Echo", blogId: 1});
        var post3 = BlogPost.remotelyCreated({id: 3, public: true, title: "Golf", blogId: 2});

        // left
        post1.remotelyUpdated({title: "Foxtrot"});
        ...
        // right
        post3.remotelyUpdated({title: "Alpha"});
        expect(updateCallback.callCount).toBe(1);
        ...
        // both
        post2.remotelyUpdated({title: "Golf"});
        expect(updateCallback.callCount).toBe(1);
    ...
    describe("remove events", function() {
      it("triggers a remove event when a record is removed from an operand and it isn't present in the other", function() {
        var post1 = BlogPost.remotelyCreated({id: 1, public: true, title: "Alpha", blogId: 1});
        var post2 = BlogPost.remotelyCreated({id: 2, public: true, title: "Bravo", blogId: 1});
        var post3 = BlogPost.remotelyCreated({id: 3, public: true, title: "Charlie", blogId: 1});

        // remove left then right
        post1.remotelyUpdated({blogId: 100, title: "Zulu"});
        ...
        post2.remotelyUpdated({public: false, title: "Zulu"});
        expect(removeCallback).not.toHaveBeenCalled();
