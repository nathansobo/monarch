describe("Monarch.Relations.Table", function() {
  ...
  describe("#getColumn(name)", function() {
    it("returns columns based on their bare or correctly-qualified names", function() {
      BlogPost.columns({
        blogId: 'integer',
        title: 'string',
        body: 'string'
  ...
  describe("#defaultOrderBy", function() {
    it("sorts records by the given specifications", function() {
      ...
      var post5 = BlogPost.created({id: 5, blogId: 3, title: "A"});
      var post1 = BlogPost.created({id: 1, blogId: 3, title: "A"});
      var post2 = BlogPost.created({id: 2, blogId: 2, title: "A"});
      var post4 = BlogPost.created({id: 4, blogId: 1, title: "B"});
      var post3 = BlogPost.created({id: 3, blogId: 1, title: "A"});
  ...
  describe("find(idOrPredicate)", function() {
    beforeEach(function() {
      BlogPost.columns({title: "string"});
    });

    describe("when passed an integer", function() {
      it("returns a record with that id or null if none is found", function() {
        var blog = BlogPost.created({id: 2});
    ...
    describe("when passed a hash", function() {
      it("returns a record matching that predicate", function() {
        var blog = BlogPost.created({id: 2, title: "Title"});
  ...
  describe("#findOrFetch(id)", function() {
    var findOrFetchSpy;
    ...
    describe("when a record with the given id already exists in the repository", function() {
      it("immediately triggers success on the promise with the record", function() {
        var blog = BlogPost.created({ id: 2, title: "Major Rip-offs" });
        BlogPost.table.findOrFetch(2).onSuccess(findOrFetchSpy);
        expect(findOrFetchSpy).toHaveBeenCalledWith(blog);
      });
    });

    describe("when no record with the given id exists in the repository", function() {
      it("fetches the record, returning the fetch promise", function() {
        BlogPost.table.findOrFetch(2).onSuccess(findOrFetchSpy);
        expect($.ajax).toHaveBeenCalled();
        expect(lastAjaxRequest.type).toBe("get");

        var record = new BlogPost({ id: 2 });
        lastAjaxRequest.success(record);
