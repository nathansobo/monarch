    ...
    describe(".create(attrs)", function() {
      ...
      describe("when the server has an error creating the record", function() {
        it("assigns validation errors on the record and marks it invalid", function() {
          ...
          var record = onInvalidCallback.mostRecentCall.args[0];
          expect(record.isA(Monarch.Record)).toBeTruthy();
          expect(record.isValid()).toBeFalsy();
          expect(record.errors.on('title')).toEqual(["Error message 1", "Error message 2"]);
          expect(record.errors.on('body')).toEqual(["Error message 3"]);
        });
      });
    });
  });
