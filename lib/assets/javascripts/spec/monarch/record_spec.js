describe("Monarch.Record", function() {
  ...
  describe("instance methods", function() {
    ...
    describe("#updated", function() {
      it("only triggers update callbacks if a change was made", function() {
    ...
    describe("#wireRepresentation", function() {
      ...
      beforeEach(function() {
        ...
        BlogPost.syntheticColumn('fooTitle', function() {
          return this.signal('title', function(title) {
            return title + " Foo";
          })
        });
      ...
      describe("when called with no arguments", function() {
        it("only includes dirty fields", function() {
    ...
    describe("#isEqual", function() {
      ...
      it("returns false for records with different ids", function() {
        var post2 = BlogPost.created({ id: 2, title: "on equality", blogId: 10 });
        expect(post1).not.toEqual(post2);
      });

      it("returns false for records of different classes", function() {
        BlogReview = new JS.Class('BlogReview', Monarch.Record);
        BlogReview.columns({
          blogId: 'integer',
          title: 'string',
          body: 'string'
        ...
        var review1 = BlogReview.created({ id: 1, title: "on equality", blogId: 10 });
        expect(post1).not.toEqual(review1);
      });

      it("returns true for records with the same id", function() {
        var otherPost1 = BlogPost.created({ id: 1 });
        expect(post1).toEqual(otherPost1);
      });
    });

    describe("#fetch", function() {
      it("fetches the record from the server", function() {
        post1 = BlogPost.created({ id: 1, title: "on equality", blogId: 10 });
        post1.fetch();

        expect($.ajax).toHaveBeenCalled();
        expect(lastAjaxRequest.type).toBe('get');
        var relation = JSON.parse(lastAjaxRequest.data.relations)[0];
