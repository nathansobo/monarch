describe("Monarch.Relations.InnerJoin", function() {
  ...
  describe("#all()", function() {
    it("returns composite tuples from the cartesian product that match the predicate", function() {
      var blog1 = Blog.created({id: 1, userId: 1, title: "Alpha"});
      var blog2 = Blog.created({id: 2, userId: 2, title: "Bravo"});
      var blog3 = Blog.created({id: 3, userId: 1, title: "Charlie"});
      var post1 = BlogPost.created({id: 1, blogId: 1, title: "Alpha Post 1"});
      var post2 = BlogPost.created({id: 2, blogId: 1, title: "Alpha Post 2"});
      var post3 = BlogPost.created({id: 3, blogId: 2, title: "Bravo Post 1"});
      var post4 = BlogPost.created({id: 4, blogId: 3, title: "Charlie Post 1"});
  ...
  describe("events", function() {
    ...
    describe("insert events", function() {
      it("triggers insert events when a tuple in either operand is inserted", function() {
        ...
        // inserts into left
        var post2 = BlogPost.created({id: 2, blogId: 2, title: "Bravo Post 1"});
        expect(insertCallback).not.toHaveBeenCalled();
        var blog2 = Blog.created({id: 2, userId: 1, title: "Bravo"});
        expect(insertCallback).toHaveBeenCalled();
        var composite = insertCallback.arg(0);
      ...
      it("triggers insert events when a tuple is updated in an operand in a way that causes it to be in the join when it wasn't previously", function() {
        var blog1 = Blog.created({id: 1, userId: 1, title: "Alpha"});
        var post1 = BlogPost.created({id: 1, blogId: 2, title: "Alpha Post 1"});
        var post2 = BlogPost.created({id: 2, blogId: 3, title: "Alpha Post 1"});
    ...
    describe("update events", function() {
      it("triggers update events when tuples in either operand are updated in a way that does not affect their membership in the join", function() {
        ...
        var blog1 = Blog.created({id: 1, userId: 1, title: "Alpha"});
        var blog2 = Blog.created({id: 2, userId: 1, title: "Bravo"});
        var blog3 = Blog.created({id: 3, userId: 2, title: "Charlie"});
        var blog4 = Blog.created({id: 4, userId: 1, title: "Delta"}); // empty
        ...
        var post4 = BlogPost.created({id: 4, blogId: 2, title: "Alpha"});
        var post5 = BlogPost.created({id: 5, blogId: 2, title: "Bravo"});
        var post6 = BlogPost.created({id: 6, blogId: 3, title: "Alpha"});
  ...
  it("has a #wireRepresentation()", function() {
    var left = Blog.where({userId: 1})
    var right = BlogPost.table;
